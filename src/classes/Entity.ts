...
import { DynamoToolbox } from '../types'
import EntityConstructor = DynamoToolbox.EntityConstructor
import FIXME = DynamoToolbox.FIXME
import ScanInputOptions = DynamoToolbox.ScanInputOptions
...
class Entity<Schema = never, HiddenKeys extends Partial<string> = never> {
  ...
  set table(table: Table) {
    ...
    if (table.Table && table.Table.attributes) {
      ...
      } else if (!table.entityNames.includes(this.name)) {
  ...
  getParams(item = {}, options: FIXME = {}, params = {}) {
  ...
  // DELETE - delete item
  async delete(item = {}, options: FIXME = {}, params = {}) {
  ...
  deleteParams(item = {}, options: FIXME = {}, params = {}) {
    // Extract schema and merge defaults
    const { schema, defaults, linked, _table } = this
    const data = normalizeData(this.DocumentClient)(
      schema.attributes,
  ...
  // UPDATE - update item
  async update(item = {}, options: FIXME = {}, params = {}) {
    // Generate the payload
      ...
      if (field === '$remove') {
        ...
        if (['number', 'set'].includes(mapping.type) && data[field].$add) {
          ...
          let value = transformAttr(
            mapping,
            validateType(mapping, field, data[field], data),
            data
          )
          ...
          if (value !== undefined) {
            ...
            SET.push(
              mapping.default && !item[field] && !mapping.onUpdate
  ...
  // PUT - put item
  async put(item = {}, options: FIXME = {}, params = {}) {
    // Generate the payload
  ...
  putParams(item = {}, options: FIXME = {}, params = {}) {
    // Extract schema and defaults
    const { schema, defaults, required, linked, _table } = this
  ...
  query(
    pk: string,
    options: DynamoToolbox.QueryInputOptions = {},
    params = {}
  ) {
  ...
  scan(options: ScanInputOptions, params = {}) {
    options.entity = this.name
